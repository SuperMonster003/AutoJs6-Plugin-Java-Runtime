...
internal object EntryClassAnalyzer {
    ...
    fun requireSingleEntry(
        ...
        val headers = try {
            ...
            throw failure(
                ...
                "ECJ produced an invalid JVM class artifact",
                error,
            )
        }
    ...
    private fun requireSingleEntry(
        ...
        if (headers.size != expectedCount) {
            throw failure(JvmSourceErrorCode.ARTIFACT_INVALID, "ECJ produced duplicate JVM class names")
        }
        ...
        fun implementsEntry(name: String, visiting: MutableSet<String>): Boolean {
            ...
            if (!visiting.add(name)) {
                throw failure(JvmSourceErrorCode.ARTIFACT_INVALID, "ECJ class hierarchy contains a cycle")
            }
        ...
        if (candidates.size != 1) {
            throw failure(
                ...
                "Java source must produce exactly one concrete AutoJsJvmEntry",
            )
        }
