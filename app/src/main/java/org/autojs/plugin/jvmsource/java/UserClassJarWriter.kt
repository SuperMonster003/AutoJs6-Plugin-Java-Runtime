...
internal object UserClassJarWriter {
    ...
    fun write(
        ...
        if (files.isEmpty() || files.size > MAX_CLASS_FILES) {
            throw failure("ECJ produced an invalid number of class files")
        }
        ...
        JarOutputStream(BufferedOutputStream(FileOutputStream(destination))).use { output ->
            files.forEach { file ->
                ...
                if (!canonicalFile.path.startsWith(prefix)) {
                    throw failure("ECJ class output escaped its private directory")
                }
                ...
                if (totalBytes > MAX_CLASS_BYTES) throw failure("ECJ class output exceeds its byte limit")
        ...
        if (!hasEntryClass) {
            ...
            throw JavaProviderFailure(
                ...
                "Java source did not produce the requested entry class",
            )
        }
